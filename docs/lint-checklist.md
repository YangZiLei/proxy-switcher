...
| 文件 | 行数 | 结论 |
|------|------|------|
| `switcher.ps1` | （见仓库；菜单 [5]/[6] 委托 launch.ps1） | 通过（Write-Host / CLI 函数名豁免仍适用） |
| `launchers/launch.ps1` | （见仓库） | 通过（Write-Host 豁免） |
| `profile/profile-functions.ps1` | （见仓库） | 通过（2 项有意豁免） |
| `scripts/ProxySwitcher.ps1` | （见仓库；共用注入） | 通过（无告警） |
| `scripts/install.ps1` | （见仓库） | 通过（1 项有意豁免） |
...
- 逐文件核对全部赋值变量均被引用：`switcher.ps1`（`$launcher`/`$pwshExe` 用于菜单 5-6）、`launch.ps1`（`$marker` / 主进程 PID）、`profile-functions.ps1`（`$cli`）、`scripts/ProxySwitcher.ps1`（`$saved`/`$pairs`）、`install.ps1`（`$shell`/`$link`）、`firewall-fix.ps1`（`$principal`/`$binDir`/`$lsPath`）、`validate.ps1`（全部）—— **均通过**。
...
- `scripts/install.ps1` 的 `-WhatIf` 参数被显式使用 —— **通过**。
...
- 进程内代理注入在 `scripts/ProxySwitcher.ps1` 的 `Invoke-WithProxySwitcherEnv`：仅设置当前进程环境变量并在 `finally` 中恢复，非持久状态变更 —— **豁免**。
